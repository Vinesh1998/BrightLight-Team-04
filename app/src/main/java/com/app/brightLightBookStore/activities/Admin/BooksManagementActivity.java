...
public class BooksManagementActivity extends AppCompatActivity {
    ...
    EditText bookName,etAuthorName,etDescription,longDesc,etAvailableBook,etPublishedDate,etPurchaseAmt,etRentalAmt,etRating;
    ...
    void validateBookDetails(){
        ...
        else if(stRentalAmt.isEmpty()) etPurchaseAmt.setError("required");
        else if(stPublished.isEmpty()) etRentalAmt.setError("required");
        else if(stRating.isEmpty()) etRating.setError("required");
        else if(stPublished.isEmpty()) etPublishedDate.setError("required");
    ...
    void addBookDetails(String auth_name, String book_name, Double purchase_amt, Double rental_amt, String shortDescription,
                        String rating, String published, int count){
        mDatabase = FirebaseDatabase.getInstance().getReference();
        String uKey = mDatabase.getDatabase().getReference("books").push().getKey();
        Book book = new Book(uKey,book_name,auth_name,downloadUrl.toString(), purchase_amt, rental_amt,shortDescription,
                rating,published,count,0);
            ...
            sRef.putFile(filePath)
                    .addOnSuccessListener(taskSnapshot -> {
                        sRef.getDownloadUrl().addOnSuccessListener(uri -> {
                             downloadUrl = uri;
                            addBookDetails(stAuthorName,stBookName,Double.parseDouble(stPurchaseAmt),Double.parseDouble(stRentalAmt),
